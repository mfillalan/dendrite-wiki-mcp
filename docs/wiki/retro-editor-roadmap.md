...
| R1: Read-only retro themes | Done | `ThemeSwitcher.vue` + `styles/retro.css` + early-paint head script. Verified end-to-end across all 4 themes. |
| R1.1: Nav chrome polish | Done | Patched `.DocSearch-Button` + Ctrl/K kbd chips, `.VPLocalNav` sub-nav strip, `.VPMenu` flyouts, `.VPNavScreen` mobile drawer, scrollbars, and the inbox-link badge ochre/red brand colors so they all inherit the active retro palette. Hid the `.VPNavBarAppearance` dark/light toggle in retro themes (the palette IS the theme). Theme-specific extras: WordPerfect's search pill inverts to deep IBM blue with yellow Ctrl/K chips; Amber gets a CRT glow on search hover. |
...
| R5: Frontmatter form | Done | Tabbed Body / Frontmatter view in `WikiEditor.vue`. Frontmatter tab renders typed inputs for `lifecycle` (select), `owner` (text), `last-reviewed` (date), `source-coverage` (select), plus a free-form key/value extras table for unknown keys. New `frontmatter-utils.ts` parses/serializes the YAML-subset losslessly. Form mutations dispatch CodeMirror transactions that replace the frontmatter region, so the editor remains the single source of truth and dirty/save logic is unchanged. Round-trip preserves unknown keys; tested by adding `review-board: design-decisions`, switching to body, switching back — key still in extras table. |
...
| R7: New-page wizard | Done | New floating "New Page" button mounts wherever you are. `NewPageWizard.vue` modal: 6 starter templates (blank, architecture, decision-record, runbook, troubleshooting, roadmap), title/slug/owner form (slug auto-fills from title; owner remembered in localStorage). Submit hands off pre-filled markdown to `WikiEditor.vue` via new `initialContent` prop. `POST /__review-bridge/pages/write` extended with create semantics: missing file + missing ifMatch = create; missing file + ifMatch = 409 (deleted); existing file + missing ifMatch = 409 (slug taken); existing file + ifMatch = normal edit. Project-log uses `Created` verb on first write, benchmark event tags `created: true`. |
| R7.1: Draft preview UX | Done | When wizard hands off to editor, `isDraft = initialContent !== undefined && !meta.hash` makes the not-yet-written state unmistakable: title prefix flips `EDIT` → `DRAFT` in warning color, `NOT SAVED YET` toast, full-width amber banner explaining "nothing has been written to disk yet", primary `F2 Save to wiki` button (brand-colored fill), `Discard draft` close button, draft-aware Esc confirm message. Auto-flips out of draft mode on first successful save. |
| Print Page button | Done | Floating "Print Page" pill (rightmost in action row, /wiki/* only) calls `window.print()`. Heavy lifting in a theme-INDEPENDENT `@media print` stylesheet so the result is identical regardless of active retro theme: chrome hidden, palette flipped to black-on-white, Special Elite typewriter typography, justified body, sensible `@page` margins, and per-row `page-break-inside: avoid` with `border-collapse: separate` to avoid trailing-border artifacts when long tables split across pages. `display: table-header-group` repeats column headers on every page when a table splits. |
