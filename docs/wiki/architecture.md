...
The MCP tool surface is shaped as three retrieval tiers so an agent can pull only as much context as it needs and pay for the rest on demand. This is the same pattern bio-inspired papers describe as "dendritic gating" — the agent gates which signals it amplifies into its context window. The framing was formalized by the bio-inspired retrieval audit on 2026-05-06 (see [Brain-Faithfulness Roadmap](./brain-faithfulness-roadmap.md) and the audit lesson under [DendriteMCP Lessons](./dendritemcp-lessons.md)) and is the canonical mental model for how Dendrite Wiki MCP serves agents.

| Tier | Purpose | Tools | Typical cost |
|---|---|---|---|
| **L0 — discovery** | Compact, ranked candidate lists. No bodies. Pure search + skill-summary surfaces. Use when the agent only needs to *know what exists*. | [wiki_search](../../src/wiki/search-index.ts), [wiki_skills_list](../../src/wiki/skill-matching.ts) | ~one-line summary per hit |
| **L1 — briefing** | Bounded task briefing assembled from ranked pages, claims, guidance, recent log entries, handoffs, memories, matching skill summaries, and the unprocessed memory backlog (B5) — with explained reasons for what was included and omitted. Use at session start and at each major task transition. | [wiki_context](../../src/wiki/store.ts), [memory_recall](../../src/wiki/memory-store.ts) | ~2–4 KB |
| **L2 — full body** | The actual markdown / skill body, fetched only for items the agent decided are worth the spend. Side effect: explicit [`wiki_skill_load`](../../src/wiki/skill-matching.ts) calls reinforce [Memory Trails](./memory-trails.md) edges with stronger weight (+0.10 vs +0.05) than passive L1 surfacing, so the L1→L2 transition is itself the success signal that feeds future ranking. Use when the agent has decided a candidate is worth the context spend. | [wiki_read](../../src/wiki/store.ts), [wiki_skill_load](../../src/wiki/skill-matching.ts) | full page / full skill body |

The agent contract in shipped guidance templates is explicit: call L1 first, then L2 only for the surfaced items the agent chose to act on. L0 is for browsing without a defined task. The tiering is what keeps `wiki_context` cheap enough to call at session start without burning the budget on content the agent will never read, while still allowing the agent to drill into full content when a candidate proves worth it.

Cross-references:

- [Skills As Memory](./skills-as-memory.md) — defines the L0 skill-summary / L2 skill-body split and the scope hard-filters that make L0 ranking deterministic.
- [Memory Trails](./memory-trails.md) — the substrate that turns L2 fetches into future L1 ranking signal via edge reinforcement.
- [Brain-Faithfulness Roadmap](./brain-faithfulness-roadmap.md) — the next track of work on the brain analogy this pattern grounds.
...
- the `docs/wiki/api/` tree, one page per source file under root `src/` and workspace `packages/*/src/`, owned by the manifest at `docs/public/api-reference-manifest.json`

The API reference tree is generated by `packages/wiki/src/api-reference.ts` (driven by the extractor + walker in `packages/wiki/src/api-extractor/`). Each page carries `lifecycle: generated` frontmatter so it stays out of the maintenance inbox, and the orphan cleanup pass deletes any slug owned by the previous manifest whose source file has been removed. The full design rationale lives in [API Reference Generation Roadmap](./api-reference-roadmap.md). For an example of what the output looks like, see the auto-generated reference for the MCP server module: [`api/server`](./api/server.md).
...
- When two code paths answer overlapping questions (e.g. "what target page would a promotion write to?" and "is the Apply button available?"), they MUST share the implementation. The bug where src/wiki/maintenance-inbox.ts had its own resolveMemoryPromotionTargetSlug copy that lacked the project-log/architecture fallback caused Apply to be permanently gated even when the actual src/wiki/memory-promotion.ts resolvePromotionTargetSlug would have succeeded. Pattern: when you find duplicated logic across modules, export the canonical version and import it everywhere. The cost of "small inline copy looks fine" compounds across releases. Specifically: the inbox availability gate must use the same target-resolution function the draft/apply paths use, so what the gate predicts and what the action does always match.
  - _Provenance: kind: lesson · recalled 87x · Sources: file:src/wiki/maintenance-inbox.ts, file:src/wiki/memory-promotion.ts_

## Promoted Lessons

- Universal MCP-side ritual reminders (the "RITUAL CHECKPOINT" footer in every tool response) are visibility, not enforcement — agents can and do ignore them under context pressure. To actually force ritual compliance in hook-capable clients (Claude Code, Codex), add a PreToolUse blocker on Edit|Write|MultiEdit|NotebookEdit that returns `permissionDecision: "deny"` until `mcp__dendrite-wiki-mcp__wiki_context` has been called for the current session, plus a Stop hook that blocks turn-end when edits happened without `wiki_log`. Key implementation details: (1) the blocker must key off Claude Code's per-chat `session_id` from hook input JSON, NOT the MCP server's internal session id (the MCP process spans many chats); (2) PostToolUse marker for `wiki_context` must check `tool_response.is_error` / "Error:" content prefix and skip marking on error, otherwise the "result too large" path becomes a silent bypass; (3) keep it alongside, not replacing, the non-blocking `skills:hook` (which surfaces matching skills, by design never blocks); (4) state file lives under `.claude/claude-code-ritual-state.json` (gitignored), reset whenever the input session_id differs from stored. Built and verified in this project at `.claude/hooks/{pre-edit-block,post-tool-mark,pre-stop-block,lib}.mjs` wired in `.claude/settings.json`.
  - _Provenance: kind: lesson · recalled 7x · Sources: file:.claude/hooks/lib.mjs, file:.claude/hooks/post-tool-mark.mjs, file:.claude/hooks/pre-edit-block.mjs, file:.claude/hooks/pre-stop-block.mjs, file:.claude/settings.json_

## Promoted Lessons

- For ritual enforcement to actually work for downstream users, three layers stack: (1) the universal MCP-side gate in src/wiki/ritual-state.ts getRitualGateRejection() refuses writing/applying tools (memory_remember, wiki_write, wiki_log, wiki_apply_proposal, etc.) until wiki_context has been called this MCP-session — this works in EVERY client (Cursor/Continue/Windsurf/Antigravity/Zed included) because every spec-compliant MCP client surfaces tool error responses to the agent; (2) per-client PreToolUse + Stop hook blockers for hook-capable clients (Claude Code, Codex, Copilot custom-agent preview) gate the file-edit vector that the MCP-side gate cannot reach; (3) src/install.ts ships the four hook scripts (lib.mjs, pre-edit-block.mjs, post-tool-mark.mjs, pre-stop-block.mjs) inlined as template strings via build*Hook() functions and writes them to downstream `&lt;project&gt;/.claude/hooks/` — both Claude Code and Codex configs reference the same path. Provide DENDRITE_DISABLE_RITUAL_GATE=1 env-var bypass in the gate so existing integration tests can drive gated tools without prepending wiki_context. The drift test in test/install.test.ts asserts the inlined strings stay byte-for-byte identical (LF-normalized) to the source-of-truth .claude/hooks/*.mjs files — fails loud if either side is edited without the other.
  - _Provenance: kind: lesson · recalled 2x · Sources: file:.claude/hooks/lib.mjs, file:.claude/hooks/post-tool-mark.mjs, file:.claude/hooks/pre-edit-block.mjs, file:.claude/hooks/pre-stop-block.mjs, file:src/install.ts, file:src/server.ts, file:src/wiki/ritual-state.ts, file:test/install.test.ts, file:test/ritual-state.test.ts_
