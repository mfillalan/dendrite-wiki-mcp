...
1. **`treeSitterExtractor`** — the long-tail language layer. Loads vendored tree-sitter WASM grammars from `vendor/tree-sitter/<lang>/` and runs each grammar's upstream `queries/tags.scm` to extract symbols and doc comments. Seven languages configured today: Rust (matches `Cargo.toml`), Go (`go.mod`), Java (`pom.xml`/`build.gradle`), Ruby (`Gemfile`), C (`Makefile`/`CMakeLists.txt`/`meson.build`), C++ (same C-family signals plus `conanfile.*`), PHP (`composer.json`). Each language config supplies its public-symbol predicate (`pub` for Rust, capitalized-name for Go, `public` modifier for Java, default-public-with-`private`/`protected`-filter for PHP, etc.), its body-node-type set for clean signature rendering, and its doc-comment style (line-prefix or block). Adding a new long-tail language is a config-table entry plus a vendored WASM/LICENSE/tags.scm tuple — no new module. WASM grammars lazy-load on first use so projects that never touch a given language never pay its load cost.
