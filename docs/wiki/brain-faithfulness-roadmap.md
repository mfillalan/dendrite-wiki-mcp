...
This roadmap originated from a 2026-05-10 strategic analysis of the Dendrite Wiki MCP memory core that identified four structural gaps and one drift asymmetry. The track shipped across two trios on the brain-faithfulness branch: the first trio (B1 memory-deposit Stop gate, B5 backlog-aware briefing banner, B7 L0/L1/L2 retrieval doc, B10 why-linter, B4 working-memory current-goal slot, B2 salience field) landed on 2026-05-10, and the second trio (B3 operator phrasebook + ritual-hook pattern matcher, B6 synaptic-pruning auto-archive, B9 sleep-cycle consolidate CLI) shipped on 2026-05-11. Nine of ten improvements are complete; B8 (page-trail bonus promotion from shadow mode) intentionally waits on recall-benchmark evidence to accumulate. The page is now the progress tracker plus design rationale — useful as the record of why each slice was built and as the acceptance-criteria reference for any future tuning, with tests across the install, wiki-store, memory-backlog, context-cache, and skill-promotion suites locking the shipped behavior.
...
- B2 (salience field with recall bonus) shipped 2026-05-10. Added optional `salience?: number` to ProjectMemoryRecord (0-3, clamped). 0=unmarked (field omitted entirely from persisted JSON to keep records clean), 1=auto-propagation floor, 2=operator-pinned important, 3=operator-pinned critical. Recall scoring in scoreProjectMemory adds Math.min(record.salience, 3) as a positive bonus with reason "salience: pinned (N)" surfaced in reasons[]. WHY this matters: brain encoding weights emotionally/consequentially salient memories higher from the moment of formation (locus-coeruleus signalling); without salience the system only marks importance via recallCount + sources, so a memory that saved hours from a bug ranks identical to a typo fix until it accrues recalls. AUTO-PROPAGATION RULE: when a new memory shares at least one relatedFile with an existing salience&gt;=2 memory, the new memory inherits salience=1 as a floor. Propagation never escalates to 2 or 3 — only direct operator pinning reaches those tiers. WHY the floor-1 ceiling: emotional weighting in real brains attenuates with associative distance; operator pinning is the canonical importance signal, propagation is a small encoding boost for memories near canonical anchors. Implementation gotchas: (1) `normalizeStoredMemoryRecord` (the JSON-deserialization path in memory-store.ts line 1406) must explicitly preserve `salience` via `...(normalizedSalience !== undefined ? { salience: normalizedSalience } : {})` — without this, seeded fixtures with `salience: 3` are silently dropped on read because the function only spreads known fields. This is THE same schema-migration gotcha that affects any future optional ProjectMemoryRecord field. (2) `normalizeSalience` lives near rememberProjectMemory and is also called by normalizeStoredMemoryRecord — keep this helper close to consumers so the contract stays visible. (3) MCP tool `memory_pin` accepts {id, salience: 0-3} and is added to GATED_TOOL_NAMES; setting salience=0 clears the field. New tool surface mirrors memory_forget/memory_restore patterns. Tests at test/memory-salience.test.ts: 9 tests (clamp, auto-propagation hit/miss/no-chain, override, pin update, missing-id returns undefined, recall ranking surfaces bonus + reason). Full sweep: 165/165 tests across install/wiki-store/memory-backlog/memory-salience/memory-why-linter/context-cache/skill-load-and-context/ritual-state/skill-promotion/diff-context/embedding-provider/review-bridge/skill-portability/memory-skill-kind/memory-ranking/mcp-server all green.
  - _Provenance: kind: lesson · recalled 4x · Sources: file:src/server.ts, file:src/wiki/memory-store.ts, file:test/memory-salience.test.ts, wiki:brain-faithfulness-roadmap_
