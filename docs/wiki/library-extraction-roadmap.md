...
- **`chart-prompts.ts` move — absorbed into slice D wave 2.** Already at `packages/wiki/src/chart-prompts.ts`.
...
**Phase 4 status: COMPLETE for the brain-and-wiki extraction milestone.** All six original phases plus the deferred items have been delivered or explicitly scoped-down with rationale. The 4-package layout is real: `npm run build -w @rarusoft/dendrite-memory` standalone, `npm run build -w @rarusoft/dendrite-wiki` standalone, all three contract tests green. Phase 5 is now in publish-prep: both extracted workspace package manifests are public-publishable, the manual npm workflow can dry-run or publish the root package, memory package, or wiki package, and the remaining validation is a real second-project dogfood.
| **5. Publish & dogfood elsewhere** | Publish `@rarusoft/dendrite-memory` and `@rarusoft/dendrite-wiki` to npm under the alpha dist-tag. Pick a second project (one you actually use) and inject the brain library. The friction the second project surfaces is the most important signal — that's the validation that the extraction is real. | None (new install on new project) | 2-3 days |
