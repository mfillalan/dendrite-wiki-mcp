...
- **Slice D — `packages/wiki/` scaffold + wiki-tier file moves.** All `src/wiki/*.ts` that did not move (store, search-index, lint, synthesis, maintenance-*, page-inbox, librarian, context-cache, generated-docs, review-bridge, canonical-target impl, etc.) relocate into `packages/wiki/src/`. Root `src/server.ts` and `src/cli.ts` stay in the umbrella package — they're the MCP adapter. Or move them into `packages/mcp-server/` (decision pending).
- **Slice E — Deferred Phase 3 renames.** `relatedPages` → `relatedDocuments` (with normalize-on-read backward compat for existing `local-data/project-memories.json`), `synthesis.ts` split into `synthesis-provider.ts` (brain) + `wiki-synthesis.ts` (wiki adapter), `chart-prompts.ts` moved to wiki.
- **Slice F — Per-package tests + CI.** Split `test/` into `packages/memory/test/` and `packages/wiki/test/` per the tier classification. Update CI workflow to run both. Verify `npm test -w @dendrite/memory` and `npm test -w @dendrite/wiki` each pass in isolation. Confirm `@dendrite/memory` builds independently (no back-references into `src/wiki/`).
