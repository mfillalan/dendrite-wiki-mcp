...
export async function installDendriteWorkspace(options: DendriteInstallOptions = {}): Promise<DendriteInstallResult> {
  ...
  if (plan.clients.includes('grok')) {
    await writeIfMissing(path.join(root, '.grok', 'config.toml'), buildGrokConfig(mode, packageName), result);
  }
...
function buildGrokConfig(mode: DendriteInstallMode, packageName: string): string {
  const serverConfig = buildProjectServerConfig(mode, packageName);
  const command = String(serverConfig.command);
  const args = Array.isArray(serverConfig.args) ? serverConfig.args.map(String) : [];

  return `# Project-scoped MCP configuration for this repository.
# Generated by dendrite-wiki init --ide grok.
...
function buildGrokSkill(): string {
...
function buildGrokHookConfig(): string {
  // We emit a JSONC-style file (with a leading comment) because many developers
  // and some loaders tolerate it, and it makes the file self-documenting.
  const comment = `// Dendrite Wiki ritual hooks for Grok Build CLI
// Generated by "dendrite-wiki init --ide grok"
// 
// This file tells Grok Build to call Dendrite's ritual hook command at key moments:
// - SessionStart: inject the full ritual contract + call wiki_context
// - UserPromptSubmit: re-inject ritual reminders + operator phrasebook nudges
// - PreToolUse (on edits): inject relevant project skills before you edit files
//
// You can safely edit or extend this file. Run "grok inspect" to verify the hooks are loaded.
`;
  ...
  const json = JSON.stringify(
    {
      hooks: {
        SessionStart: [
          {
            ...
            hooks: [
              {
                type: "command",
                command: "dendrite-wiki ritual:grok-hook",
                timeout: 30
              }
            ]
          }
        ...
        PreToolUse: [
          {
            matcher: "Edit|Write|MultiEdit|run_terminal_cmd",
            hooks: [
              {
                type: "command",
                command: "dendrite-wiki ritual:grok-hook",
...
type InstallClient = 'vscode' | 'cursor' | 'claude' | 'codex' | 'continue' | 'windsurf' | 'antigravity' | 'grok';
type InstallAsset =
  | 'agents-file'
...
function buildInstallPlan(profile: DendriteInstallProfile): { clients: InstallClient[]; assets: InstallAsset[] } {
  ...
  if (profile === 'grok') {
    return {
      clients: ['grok'],
      assets: ['grok-skill', 'grok-hooks']
    };
  }

  return {
    clients: ['vscode', 'cursor', 'claude', 'codex', 'continue', 'grok'],
    assets: [
      'agents-file',
      'copilot-instructions',
      'vscode-instructions',
