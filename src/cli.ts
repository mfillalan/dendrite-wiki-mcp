...
const installProfiles: DendriteInstallProfile[] = ['all', 'claude', 'copilot-vscode', 'cursor', 'codex', 'continue', 'windsurf', 'antigravity', 'grok'];
const autoInstallAliases = new Set(['auto', 'detect']);
...
try {
  if (!command || command === 'help' || command === '--help' || command === '-h') {
    ...
    const report = await runDoctor({ writeTelemetryStatus: args.includes('--write-telemetry-status') });
    if (asJson) {
      console.log(JSON.stringify(report, null, 2));
    } else {
      console.log(formatDoctorReport(report));
      if (!args.includes('--write-telemetry-status')) {
        console.log('Read-only audit: no generated status artifacts were written. Pass --write-telemetry-status to refresh the telemetry status JSON.');
      }
    }
...
function readProfile(args: string[]): DendriteInstallProfile {
  ...
  if (explicitIde) {
    ...
    if (autoInstallAliases.has(normalized)) {
      return detectInstallProfile();
    }
    const mapped = ideAliasToProfile[normalized];
    if (!mapped) {
      throw new Error(
        `Unsupported --ide value: ${explicitIde}. Known IDEs: ${[...Object.keys(ideAliasToProfile), ...autoInstallAliases].sort().join(', ')}.`
      );
    }
    return mapped;
  }

  const explicitProfile = readValue(args, '--profile');
  if (!explicitProfile) {
    return 'all';
  }

  const normalizedProfile = explicitProfile.trim().toLowerCase();
  if (autoInstallAliases.has(normalizedProfile)) {
    return detectInstallProfile();
  }

  if (!installProfiles.includes(normalizedProfile as DendriteInstallProfile)) {
    throw new Error(`Unsupported init profile: ${explicitProfile}`);
  }

  return normalizedProfile as DendriteInstallProfile;
}

function detectInstallProfile(root = process.cwd()): DendriteInstallProfile {
  const signals: Array<{ profile: DendriteInstallProfile; paths: string[] }> = [
    { profile: 'claude', paths: ['.claude'] },
    { profile: 'codex', paths: ['.codex'] },
    { profile: 'cursor', paths: ['.cursor'] },
    { profile: 'continue', paths: ['.continue'] },
    { profile: 'grok', paths: ['.grok'] },
    { profile: 'copilot-vscode', paths: ['.github/agents', '.github/copilot-instructions.md'] }
  ];

  const matches = signals.filter((signal) =>
    signal.paths.some((relativePath) => existsSync(path.join(root, relativePath)))
  );
