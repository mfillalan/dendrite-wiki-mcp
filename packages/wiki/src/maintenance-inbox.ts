...
/**
 * Best-effort extraction of the suggested refreshed paragraph that we now embed
 * in page-drift lint messages (via suggestRefreshedPageIntent in the store).
 */
function extractSuggestedRefreshFromDriftMessage(message: string | undefined): string | null {
  if (!message) return null;
  const marker = 'Suggested refreshed intent:';
  const idx = message.indexOf(marker);
  if (idx === -1) return null;
  const rest = message.slice(idx + marker.length).trim();
  // Take until the next blank line or end of the relevant section.
...
function buildLintActions(finding: WikiLintFinding): MaintenanceInboxActionHint[] {
  ...
  if (finding.rule === 'page-drift' && wikiSlug) {
    ...
    // For page-drift, try to provide the actual high-quality suggestion generated by
    // suggestRefreshedPageIntent (now embedded in the finding message).
    const suggestedFromMessage = extractSuggestedRefreshFromDriftMessage(finding.message);
    const driftHint = suggestedFromMessage || 
      ((finding.message || '').includes('Recent activity tokens:')
