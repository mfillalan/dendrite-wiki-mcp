...
function createTransport(cwd: string, benchmarkEvents: 'enabled' | 'disabled' = 'disabled'): StdioClientTransport {
  return new StdioClientTransport({
    ...
    env: {
      ...
      // Bypass the B10 why-linter in integration tests. Many fixtures use bare lesson
      // bodies without causal language; the linter itself is covered by dedicated tests
      // in test/memory-why-linter.test.ts.
      DENDRITE_DISABLE_WHY_LINTER: '1'
    }
  });
}
...
test('MCP server exposes and serves the wiki tool surface over stdio', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ['memory_accept_supervision_proposal', 'memory_add_open_question', 'memory_auto_archive', 'memory_auto_clean_apply', 'memory_auto_clean_revert', 'memory_auto_clean_runs', 'memory_forget', 'memory_handoff', 'memory_list_supervision_proposals', 'memory_mark_decided', 'memory_mark_deferred', 'memory_pin', 'memory_promote', 'memory_promote_skill', 'memory_recall', 'memory_reject_supervision_proposal', 'memory_remember', 'memory_restore', 'memory_review', 'memory_set_goal', 'memory_trigger_satisfied', 'skill_export', 'skill_import', 'wiki_apply_proposal', 'wiki_context', 'wiki_execute_maintenance_action', 'wiki_generate_api_reference', 'wiki_graph', 'wiki_index', 'wiki_insert_chart', 'wiki_librarian_audit', 'wiki_lint', 'wiki_log', 'wiki_maintenance_inbox', 'wiki_proposals', 'wiki_read', 'wiki_replace_chart', 'wiki_search', 'wiki_skill_load', 'wiki_skills_list', 'wiki_synthesize_claims', 'wiki_synthesize_guidance', 'wiki_synthesize_proposals', 'wiki_write', 'wiki_write_proposals']
    );
